...
Optimize for correct outcomes with minimal changes, minimal tokens, and clear traceability. Read before writing. Verify after changing.
...
2. **Plan** — define concrete steps; surface blockers before starting
3. **Execute** — focused, surgical edits; no unrelated refactors
4. **Verify** — run the narrowest meaningful check for the changed scope
5. **Report** — summarize `result`, `changes_made`, `validation`, `open_items`, `sources_or_files`
...
**Biome + TypeScript conflict on `Record<string, unknown>`:** Biome wants dot notation, TypeScript requires bracket notation for index-signature types. Resolution: use a typed interface with named properties. See `AiderYamlConfig` in [packages/adapters/aider/src/index.ts](packages/adapters/aider/src/index.ts).
...
`z.looseObject()` produces a type with `[key: string]: unknown`. When accessing named properties on its inferred type, cast first: `const x = doc.frontmatter.tools?.aider as AiderOverrides | undefined`.

---

## Adding a New Adapter

1. Scaffold package under `packages/adapters/<tool-id>/` with `package.json`, `tsconfig.json` (references `../../core`), `src/index.ts`, `src/__tests__/index.test.ts`
2. Implement `ToolAdapter` from `@laup/core` (`toolId`, `displayName`, `render()`, `write()`)
3. Create golden files: `src/__tests__/golden/canonical-input.md` + expected output(s)
4. Add to `ALL_ADAPTERS` in `packages/cli/src/bin.ts` and add dep in `packages/cli/package.json`
5. `pnpm install && pnpm run build && pnpm run test:run && pnpm run typecheck && pnpm run lint`
...
- Tests for config-hub and CLI are integration tests — they write to `os.tmpdir()` and verify output files
...
Scopes: `core`, `cli`, `config-hub`, `adapters/claude-code`, `adapters/cursor`, `adapters/aider`.
...
Architecture docs live in `/Users/mattmoore/untitled/docs/laup/`. When a task depends on architecture decisions:

- Route by area: `phase-2-architecture/DOC-20x-*` for design, `phase-1-requirements/DOC-103-*` for requirements
...
Memory and cross-session context: `/Users/mattmoore/.claude/projects/-Users-mattmoore-untitled/memory/MEMORY.md`
